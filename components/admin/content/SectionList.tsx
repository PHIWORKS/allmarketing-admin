...
export default function SectionList({ section }: { section: Section }) {
  ...
  const [data, setData] = useState<{ total: number; rows: any[] }>({ total: 0, rows: [] });
  ...
  const fetchList = async () => {
    setLoading(true);
    const qs = new URLSearchParams({ q, status, access, page: String(page), size: "10" });
    const res = await fetch(`/api/admin/content/${section}?${qs}`);
    const j = await res.json();
    setData(j);
    setLoading(false);
  };
  useEffect(() => { fetchList(); /* eslint-disable-next-line */ }, [q, status, access, page, section]);
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="rounded-xl border bg-white p-3 flex flex-wrap gap-2">
        ...
        />
        <select className="border rounded-lg px-3 py-2" value={status} onChange={(e)=>setParam("status", e.target.value)}>
        ...
        <select className="border rounded-lg px-3 py-2" value={access} onChange={(e)=>setParam("access", e.target.value)}>
      ...
      <div className="rounded-2xl border bg-white overflow-x-auto">
        <table className="min-w-full text-sm">
          ...
          <tbody>
            {loading && <tr><td className="px-4 py-6 text-center text-gray-400" colSpan={8}>로딩중…</td></tr>}
              ...
              <tr key={r.id} className="border-t">
                ...
                <td className="px-4 py-3">
                  <Link className="text-emerald-600 hover:underline"
                        href={`/admin/content/${section}/${r.id}/edit`}>수정</Link>
                </td>
              </tr>
            ))}
            {!loading && data.rows.length===0 && (
              <tr><td className="px-4 py-6 text-center text-gray-400" colSpan={8}>데이터가 없습니다</td></tr>
            )}
          </tbody>
        </table>
      </div>
